...
const VENUE_ADDRESS = "경기도 수원시 권선구 온정로 72";
const MAP_URL = `https://map.naver.com/p/search/${encodeURIComponent(VENUE_ADDRESS)}`;

const programInfoSections = [
  {
    title: "작가소개",
    body: "와즈디 무아와드는 레바논 출신의 캐나다 극작가이자 연출가입니다. 그의 작품은 전쟁, 기억, 가족, 침묵의 문제를 시적인 언어와 강렬한 서사로 풀어냅니다.",
  },
  {
    title: "작품소개",
    body: "〈화염〉은 한 가족의 비밀을 추적하는 과정을 통해 전쟁이 개인의 삶에 남긴 상처와 기억을 마주하게 하는 현대희곡입니다.",
  },
  {
    title: "줄거리소개",
    body: "쌍둥이 남매 잔느와 시몽은 어머니 나왈의 유언을 따라 아버지와 형제를 찾아 나섭니다. 그 여정 속에서 두 사람은 어머니의 침묵 뒤에 숨겨진 진실과 마주하게 됩니다.",
  },
  {
    title: "연출의도",
    body: `〈화염〉을 준비하며 ‘왜 나왈은 침묵할 수밖에 없었는가’라는 질문에 주목했습니다.
본 공연은 전쟁 자체보다 그 안에서 살아가는 여성들의 삶과 억압에 집중합니다.
나왈의 침묵이 단순한 비밀이 아니라 살아남기 위한 선택이었음을 무대 위에 드러내고자 합니다.
관객들이 그 침묵에 담긴 의미를 함께 고민할 수 있기를 바랍니다.`,
  },
  {
    title: "기획의도",
    body: "이 공연은 관객이 나왈의 침묵을 단순한 비밀이 아닌 삶의 흔적으로 바라보게 만드는 데 목적이 있습니다. 공연 전후로 인물과 제작진을 함께 기억할 수 있도록 이 아카이브를 구성했습니다.",
  },
];
...
function runDataChecks() {
  ...
  console.assert(programInfoSections.length === 5, `프로그램 정보는 5개여야 합니다. 현재 ${programInfoSections.length}개입니다.`);
  console.assert(programInfoSections.every((section) => section.title && section.body), "모든 프로그램 정보에는 title과 body가 있어야 합니다.");
  ...
  console.assert(MAP_URL.includes(encodeURIComponent(VENUE_ADDRESS)), "지도 링크에는 공연장 주소가 포함되어야 합니다.");
}
...
function ProgramInfoModal({ selected, onClose }) {
  return (
    <AnimatePresence>
      {selected && (
        <motion.div
        ...
        >
          <motion.div
            ...
            transition={{ type: "spring", stiffness: 260, damping: 24 }}
            onClick={(event) => event.stopPropagation()}
            className="relative w-full max-w-[390px] overflow-hidden rounded-3xl border border-white/10 bg-[#120d0b] shadow-2xl"
          >
            <button
              type="button"
              onClick={onClose}
            ...
            <div className="bg-[radial-gradient(circle_at_20%_10%,rgba(239,68,68,0.32),transparent_32%),linear-gradient(135deg,#2b0b08,#090605)] p-7 pt-10">
              <p className="mb-3 text-xs font-bold uppercase tracking-[0.28em] text-red-300">Program Note</p>
              <h3 className="text-3xl font-black text-white">{selected.title}</h3>
            </div>
            <div className="p-7">
              <p className="whitespace-pre-line text-base leading-8 text-stone-200">{selected.body}</p>
            </div>
          </motion.div>
        </motion.div>
      )}
...
export default function PerformanceProfilePromoSite() {
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-[430px] overflow-hidden bg-[#0b0807] text-stone-100 shadow-2xl shadow-black/40">
              ...
              <div className="min-h-[420px] overflow-hidden rounded-[1.5rem] bg-[radial-gradient(circle_at_40%_20%,rgba(254,202,202,0.24),transparent_28%),linear-gradient(135deg,#2b0b08,#090605_55%,#581c1c)] p-6">
                <div className="flex h-full flex-col justify-between">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2">
                    {programInfoSections.map((section) => (
                      <button
                        key={section.title}
                        type="button"
                        onClick={() => setSelectedProgramInfo(section)}
                        className="min-w-0 rounded-2xl border border-red-200/10 bg-white/[0.06] px-3 py-3 text-center text-xs font-bold leading-tight text-stone-100 transition hover:bg-white/10"
                      >
                        {section.title}
                      </button>
                    ))}
      ...
      <section id="info" className="border-y border-white/10 bg-white/[0.03] px-5 py-8">
        <div className="mx-auto grid max-w-7xl gap-4">
          ...
          <div className="rounded-3xl border border-white/10 bg-black/20 p-6">
            ...
            <p className="mt-2 text-sm leading-6 text-stone-300">{VENUE_ADDRESS}</p>
            <a
              href={MAP_URL}
              target="_blank"
              rel="noreferrer"
              className="mt-4 inline-flex items-center justify-center gap-2 rounded-full border border-white/10 bg-white/[0.06] px-5 py-3 text-sm font-bold text-white transition hover:bg-white/10"
            >
              <MapPin size={16} /> 지도보기
            </a>
          </div>
      ...
      <ProgramInfoModal selected={selectedProgramInfo} onClose={() => setSelectedProgramInfo(null)} />
    </main>
  );
}
